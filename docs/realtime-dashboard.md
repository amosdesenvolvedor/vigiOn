...
Há no máximo três conexões por usuário, heartbeat SSE a cada 20 segundos e polling de fallback a cada 60 segundos. A conexão possui duração máxima de 15 minutos; ao encerrá-la, o browser precisa obter novo ticket passando novamente pela autenticação e pela validação da sessão. Clientes lentos cuja escrita aplica backpressure são desconectados. A implementação é single-instance e em memória; múltiplas réplicas exigirão pub/sub compartilhado no Prompt 17.

O summary agrega contagens tenant-scoped, até 50 câmeras, 20 gateways, 20 eventos e 20 alertas, com includes controlados. A grade nunca inicia streams automaticamente. O usuário abre uma câmera online explicitamente, reutilizando `StreamSession`, player HLS seguro e limites existentes. Fechar o player ou desmontá-lo (inclusive em logout/navegação) solicita encerramento da sessão; o TTL do backend continua sendo a garantia final após perda abrupta de rede.
